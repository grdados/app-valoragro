...
export default function CoordenadoresPage() {
  ...
  const openCreate = () => {
    setEditing(null)
    reset({ ativo: true, supervisor: null })
    setModalOpen(true)
  }

  const openEdit = (item: Coordenador) => {
    setEditing(item)
    reset(item)
    setModalOpen(true)
  }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (editing) {
        await coordenadoresApi.update(editing.id, data)
        toast.success('Coordenador atualizado!')
      } else {
        await coordenadoresApi.create(data)
  ...
  const handleDelete = async (item: Coordenador) => {
    ...
    try {
      await coordenadoresApi.remove(item.id)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Coordenadores"
        subtitle="Gestão de coordenadores de vendas"
        actions={
      ...
      <Modal
        ...
        title={editing ? 'Editar Coordenador' : 'Novo Coordenador'}
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            {errors.supervisor && <p className="text-red-500 text-xs mt-1">Obrigatório</p>}
          </div>
