...
import { faixasCoordenadorApi } from '../../services/api'
...
export default function FaixasCoordenadorPage() {
  ...
  const fetch = async () => {
    setLoading(true)
    try {
      const res = await faixasCoordenadorApi.list()
      ...
      if (status === 404) {
        toast.error('Endpoint de comissão do coordenador não encontrado no backend. Atualize backend + migrações.')
      } else {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (editing) {
        await faixasCoordenadorApi.update(editing.id, payload)
        toast.success('Faixa atualizada!')
      } else {
        await faixasCoordenadorApi.create(payload)
  ...
  const handleDelete = async (item: FaixaComissaoPerfil) => {
    ...
    try {
      await faixasCoordenadorApi.remove(item.id)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Comissão do Coordenador"
        subtitle="Faixas por valor do bem para cálculo da comissão do coordenador"
        actions={
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Faixa' : 'Nova Faixa do Coordenador'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="ativo_coordenador" {...register('ativo')} className="w-4 h-4" />
            <label htmlFor="ativo_coordenador" className="text-sm">Ativo</label>
          </div>
