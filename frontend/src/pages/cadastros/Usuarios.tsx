...
interface SystemUser {
  ...
  perfil: string
  is_active: boolean
  supervisor_ref: number | null
  coordenador_ref: number | null
  vendedor_ref: number | null
}
...
export default function UsuariosPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [u, c, v, s] = await Promise.all([
        usuariosApi.list(),
        coordenadoresApi.list(),
        vendedoresApi.list(),
        supervisoresApi.list(),
      ])
  ...
  const openCreate = () => {
    setEditing(null)
    reset({ is_active: true, perfil: 'vendedor' })
    setModalOpen(true)
  }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (data.perfil === 'supervisor' && !data.supervisor_ref) {
        toast.error('Selecione o supervisor vinculado para este usuário.')
        return
      }
      if (data.perfil === 'coordenador' && !data.coordenador_ref) {
        toast.error('Selecione o coordenador vinculado para este usuário.')
        return
      }
  ...
  const handleDelete = async (item: SystemUser) => {
    ...
    try {
      await usuariosApi.remove(item.id)
  ...
  const columns = [
    { key: 'username', header: 'Usuário' },
    {
      key: 'first_name',
      header: 'Nome',
      render: (row: SystemUser) => `${row.first_name} ${row.last_name}`.trim(),
    },
    { key: 'email', header: 'E-mail' },
    {
      ...
      render: (row: SystemUser) => (
        <span className="badge-ativa">{perfilLabels[row.perfil] || row.perfil}</span>
      ),
    },
    {
      key: 'is_active',
      header: 'Status',
      render: (row: SystemUser) => (
        <span className={row.is_active ? 'badge-ativa' : 'badge-cancelada'}>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.username && <p className="text-red-500 text-xs mt-1">Obrigatório</p>}
          </div>
          ...
          <div>
            ...
            {errors.password && (
              <p className="text-red-500 text-xs mt-1">Obrigatório para novos usuários</p>
            )}
          </div>
          ...
          {perfilWatch === 'supervisor' && (
            <div>
              ...
              {errors.supervisor_ref && <p className="text-red-500 text-xs mt-1">Obrigatório</p>}
            </div>
          )}
          ...
          {perfilWatch === 'coordenador' && (
            <div>
              ...
              {errors.coordenador_ref && <p className="text-red-500 text-xs mt-1">Obrigatório</p>}
            </div>
          )}
