...
interface FormData { consorcio: number; valor_min: number; valor_max: number; percentuais_str: string; ativo: boolean }
...
export default function FaixasPage() {
  ...
  const openEdit = (item: FaixaComissao) => {
    setEditing(item)
    reset({ ...item, percentuais_str: item.percentuais.join(', ') })
    setModalOpen(true)
  }
  ...
  const onSubmit = async (data: FormData) => {
    setSaving(true)
    try {
      const percentuais = data.percentuais_str.split(',').map((p) => parseFloat(p.trim())).filter((p) => !isNaN(p))
      const payload = { consorcio: data.consorcio, valor_min: data.valor_min, valor_max: data.valor_max, percentuais, ativo: data.ativo }
      if (editing) { await faixasApi.update(editing.id, payload); toast.success('Faixa atualizada!') }
      else { await faixasApi.create(payload); toast.success('Faixa criada!') }
      setModalOpen(false)
      fetch()
    } catch (err: unknown) {
  ...
  const handleDelete = async (item: FaixaComissao) => {
    ...
    try { await faixasApi.remove(item.id); toast.success('Removida!'); fetch() }
  ...
  const columns = [
    ...
    { key: 'ativo', header: 'Status', render: (row: FaixaComissao) => <span className={row.ativo ? 'badge-ativa' : 'badge-cancelada'}>{row.ativo ? 'Ativo' : 'Inativo'}</span> },
  ]

  return (
    <div className="space-y-6">
      <PageHeader title="Faixas de Comissão" subtitle="Configuração de percentuais por faixa de valor" actions={<button onClick={openCreate} className="btn-primary"><Plus className="w-4 h-4" />Nova Faixa</button>} />
      <DataTable columns={columns} data={items} keyField="id" loading={loading} emptyMessage="Nenhuma faixa cadastrada"
        actions={(row) => (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Faixa' : 'Nova Faixa de Comissão'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="label">Valor Mínimo (R$) *</label><input type="number" step="0.01" {...register('valor_min', { required: true })} className="input" /></div>
            <div><label className="label">Valor Máximo (R$) *</label><input type="number" step="0.01" {...register('valor_max', { required: true })} className="input" /></div>
          </div>
          <div>
            <label className="label">Percentuais por Parcela *</label>
            <input {...register('percentuais_str', { required: true })} className="input" placeholder="Ex: 0.5, 0.5, 0.5 (separados por vírgula)" />
            <p className="text-xs text-gray-400 mt-1">Informe o percentual de cada parcela separado por vírgula. Ex: 0.5, 0.5 para 2 parcelas de 0.5%</p>
          </div>
