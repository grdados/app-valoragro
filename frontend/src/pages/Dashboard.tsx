...
export default function DashboardPage() {
  ...
  const adminData = (data || {}) as Partial<DashboardAdmin>
  const vendMesData = Array.isArray((data as DashboardVendedor | DashboardAdmin)?.vendas_por_mes)
    ? (data as DashboardVendedor | DashboardAdmin).vendas_por_mes
    : []
  ...
  const rankingVendasChartData = (
    Array.isArray(adminData?.ranking_vendedores) ? adminData.ranking_vendedores : []
  ).map((item) => ({
    vendedor: item.vendedor__nome,
    total: Number(item.total_vendas || 0),
    qtd: Number(item.qtd_vendas || 0),
  }))

  const vendasPorProduto = (Array.isArray(adminData?.vendas_por_tipo) ? adminData.vendas_por_tipo : []).map(
    (item) => ({
  ...
  const vendasPorConsorcio = (
    Array.isArray(adminData?.vendas_por_consorcio) ? adminData.vendas_por_consorcio : []
  ).map((item) => ({
  ...
  const vendasMensalPorProdutoPivot = useMemo(() => {
    const rows = Array.isArray(adminData?.vendas_mensal_por_produto)
      ? adminData.vendas_mensal_por_produto.slice()
      : []
    ...
    if (!rows.length) {
      return {
        ...
        produtos: [] as string[],
        produtoKeyMap: new Map<string, string>(),
      }
    }

    const produtos = Array.from(new Set(rows.map((row) => row.produto || 'Sem produto')))
    const produtoKeyMap = new Map<string, string>()
    produtos.forEach((produto, idx) => {
      produtoKeyMap.set(produto, `produto_${idx}`)
    })
    ...
    rows.forEach((row) => {
      ...
      if (!mesIso) return
      const produtoNome = row.produto || 'Sem produto'
      const produtoKey = produtoKeyMap.get(produtoNome) || 'produto_0'
      ...
      current[produtoKey] = Number(row.total || 0)
    })
    ...
    const dataPivot = Array.from(byMonth.values())
      ...
      .map((entry) => {
        ...
        produtos.forEach((produto) => {
          const key = produtoKeyMap.get(produto) || 'produto_0'
          if (normalized[key] === undefined) normalized[key] = 0
        })
        return normalized
      })

    return { data: dataPivot, produtos, produtoKeyMap }
        ...
        <>
          ...
          {adminData?.total_contemplados !== undefined && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <StatCard title="Valor Total Contemplado" value={formatCurrency(Number(adminData.total_contemplados || 0))} icon={Star} color="green" />
              <StatCard title="Consórcios Contemplados" value={`${Number(adminData.qtd_contemplados || 0)} contratos`} icon={Award} color="purple" />
            </div>
          )}
                ...
                <LineChart data={vendasMensalPorProdutoPivot.data}>
                  ...
                  {vendasMensalPorProdutoPivot.produtos.map((produto, index) => (
                    <Line
                      key={produto}
                      type="monotone"
                      dataKey={vendasMensalPorProdutoPivot.produtoKeyMap.get(produto) || 'produto_0'}
                      name={produto}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            {Array.isArray(adminData?.vendas_por_coban) && adminData.vendas_por_coban.length > 0 && (
              <div className="card p-6">
